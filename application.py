...
def process_image(img_path):
    ...
    disp_resized = torch.nn.functional.interpolate(disparity, (original_height, original_width), mode="bilinear", align_corners=False)
...
def process_video(vid_path):
    ROOT_DIR = os.path.dirname(os.path.abspath(__file__))
    input_filename = os.path.join(ROOT_DIR + f'/static/input.mp4')
    output_filename = os.path.join(ROOT_DIR + f'/static/ouput.mp4')
